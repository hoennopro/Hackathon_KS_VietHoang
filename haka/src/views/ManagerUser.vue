<template>
    <div class="manager-user-container">
      <div class="header">
        <h2>Manager User</h2>
        <button class="add-user-button" @click="openAddUserForm">Add New User</button>
      </div>
      <AddUserForm
      ...
      <table class="user-table">
        ...
        <tbody>
          <tr v-for="(user, index) in users" :key="index">
            ...
            <td>{{ user.email }}</td>
            <td>
              <button class="edit-button" @click="openEditUserForm(user)">Edit</button>
  ...
  const index = users.value.findIndex((user) => user.email === updatedUser.email);
  if (index !== -1) {
